...
import novelty_dataset_wrappers as dwrap
...
parser.add_argument('--general_config_path', type=str, default='../configs_new/incDFM_1by1_CL_modifiable_config.yaml')
...
for t in range(num_tasks):
    ...
    if t>0:
        ...
        if args.threshold_type=='simple':
            ...
            test_save_scores = np.vstack([results_novelty_test.scores, results_novelty_test.gt_novelty, results_novelty_test.gt])
            seen_save_scores = np.vstack([results_novelty.scores, results_novelty.gt_novelty, results_novelty.gt])
            np.save('/lab/arios/ProjIntel/incDFM/sandbox/test_scores_rebuttal_2_2_%s.npy'%args.novelty_detector_name, test_save_scores)
            np.save('/lab/arios/ProjIntel/incDFM/sandbox/seen_scores_rebuttal_2_2_%s.npy'%args.novelty_detector_name, seen_save_scores)
            sys.exit()
