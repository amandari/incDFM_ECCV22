...
import novelty_dfm_CL.dset8_specific_scripts.novelty_dataset_wrappers_8dset as dwrap

import novelty_dfm_CL.dset8_specific_scripts.datasets_holdout_validation_8dset as dseth
import novelty_dfm_CL.dset8_specific_scripts.novelty_detector_8dset as novel
import novelty_dfm_CL.dset8_specific_scripts.novelty_eval_8dset as novelval 
import novelty_dfm_CL.dset8_specific_scripts.classifier_8dset as clf
import argparse

## For optional Arguments 
parser = argparse.ArgumentParser(description='incDFM and baselines')
parser.add_argument('--general_config_path', type=str, default='../configs_new/incDFM_8dset_CL_modifiable_config.yaml')
...
if (args.novelty_detector_name=='odin' or args.novelty_detector_name=='softmax') or args.train_clf==True:
    train_clf = True
else:
...
datasets_use = dseth.call_8dset_holdout_w_validation(args.data_dir, args.experiment_dir, num_tasks=args.num_tasks,\
                                        ...
                                        use_coarse_data=args.use_coarse_data)

train_datasets, train_holdout_datasets, val_datasets, test_datasets, list_tasks, list_tasks_finegrained, dset_prep = datasets_use
...
test_loaders = [torch.utils.data.DataLoader(test_datasets[t], batch_size=args.batchsize_test_clf,
...
if len(args.fc_sizes)>0:
    ...
    fc_sizes = []
network = clf.Resnet_PSP(dset_prep['num_classes_fine_tasks'], num_tasks, resnet_arch=args.net_type, FC_layers=fc_sizes, base_freeze=True)
...
# 3) ---- Novelty
if args.use_output_mlp_coarse:
    total_classes = dset_prep['total_classes_coarse']
else:
    total_classes = dset_prep['total_classes_fine']
...
if args.novelty_detector_name=='odin':
    args.detector_params['num_tasks']=num_tasks #simple network (not wrapper) - Is this problematic? TODO
    args.detector_params['base_network']=network #simple network (not wrapper) - Is this problematic? TODO
    args.detector_params['num_classes_fine_tasks']=dset_prep['num_classes_fine_tasks']
    ...
    args.detector_params['feature_extractor']=network_inner #simple network (not wrapper) - Is this problematic? TODO
...
for t in range(num_tasks):
    ...
    if t>0:
        ...
        current_dset = dwrap.CurrentTask(new_data, use_old_dsets[:t], use_coarse=dset_prep['use_coarse_data'])
        current_loader = torch.utils.data.DataLoader(current_dset, batch_size=args.batchsize_test_clf,
        ...
        test_old_plus_novel_dset = dwrap.CurrentTask(test_datasets[t], use_old_dsets_test[:t], use_coarse=dset_prep['use_coarse_data'])
        test_old_plus_novel_loader = torch.utils.data.DataLoader(test_old_plus_novel_dset, batch_size=args.batchsize_test_score,
                                                shuffle=False, num_workers=args.num_workers)
        
        # print('test_loaders[0].dataset.__len__()', test_loaders[0].dataset.__len__())

        
        params_score = {'layer':args.dfm_layers_input, \
            'base_apply_score':True, 'target_ind':dset_prep['scenario_classif'], 'device': args.device, 'current_task':t}
        
        
        # make validation wrapper 
        val_loader = torch.utils.data.DataLoader(dwrap.ValidationDset(val_datasets[:t], use_coarse=dset_prep['use_coarse_data']), batch_size=args.batchsize_test_score,
                                                shuffle=False, num_workers=args.num_workers)
        ...
        if args.threshold_type=='simple':
            ...
            if args.task_oracle:
                ## If using a task oracle
                params_score_unsup = copy.deepcopy(params_score)
                params_score_unsup['true_task']='known'
            else:
                params_score_unsup = copy.deepcopy(params_score)
                params_score_unsup['true_task']=None
            
            
            params_score_val = copy.deepcopy(params_score)
            params_score_val['true_task']='known'
            
            
            # 2) Threshold Novel/Old - Binary prediction 
            if args.novelty_detector_name=='odin':
                current_loader_score = torch.utils.data.DataLoader(current_dset, batch_size=args.batchsize_test_score,
                                                shuffle=False, num_workers=args.num_workers)
                results_novelty = novelval.evaluate_odin_CL(t, novelty_detector, noveltyResults, params_score_unsup, current_loader_score)
                params_score_val['score_func'] = results_novelty.best_score_func
                params_score_val['noise_magnitude'] = results_novelty.best_noise_magnitude
                # invert scores for ODIN so that high scores are novelty (1)
            elif args.novelty_detector_name=='softmax':
                current_loader_score = torch.utils.data.DataLoader(current_dset, batch_size=args.batchsize_test_score,
                                                shuffle=False, num_workers=args.num_workers)
                results_novelty = novelval.evaluate_simple_CL(t, novelty_detector, noveltyResults, params_score_unsup, \
                    current_loader_score, args.novelty_detector_name)
            else:
                results_novelty = novelval.evaluate_simple_CL(t, novelty_detector, noveltyResults, params_score_unsup, \
            ...
            threshold_estimated_novel = novel.estimate_threshold_from_val_simple(val_loader, args.percentile_val_threshold, \
                novelty_detector, params_score_val, args.novelty_detector_name)   
            

            # threshold_estimated_novel TODO
            inds_pred_novel,_ = novel.Threshold_n_Select_Simple(t, results_novelty, threshold_estimated_novel, \
            ...
            novelval.evaluate_simple_CL_test(t, novelty_detector, noveltyResults_test, params_score_unsup, test_old_plus_novel_loader, args.novelty_detector_name)
        ...
        if args.prediction_propagation:
            if inds_pred_novel.shape[0]>0:
                ...
                loader_new = torch.utils.data.DataLoader(dwrap.NovelTask(t, num_classes, current_dset, pred_novel_inds=inds_pred_novel, \
                    use_coarse=dset_prep['use_coarse_data']), batch_size=batchsize_new,
            ...
            loader_new = torch.utils.data.DataLoader(dwrap.NovelTask(t, num_classes, current_dset, pred_novel_inds=inds_gt_novel, \
                use_coarse=args.use_output_mlp_coarse), batch_size=batchsize_new,
                                                    shuffle=True, num_workers=args.num_workers)
            
    else:
        loader_new = torch.utils.data.DataLoader(dwrap.NovelTask(t, num_classes, train_datasets[t], use_coarse=args.use_output_mlp_coarse), \
            batch_size=batchsize_new, shuffle=True, num_workers=args.num_workers)
        



    # print('batch example', next(iter(loader_new)))
    ...
    if train_clf:
        clf.train(t, args, novelty_detector, network_inner, loader_new, test_loaders, train_technique=args.train_technique)
    ...
    if loader_new is not None:
        ...
        if t<num_tasks-1:
            ...
            if (args.novelty_detector_name=='mahal') or (args.novelty_detector_name=='dfm') or (args.novelty_detector_name=='incdfm'):
