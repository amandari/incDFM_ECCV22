...
def call_8dset_holdout_w_validation(data_dir, experiment_dir, experiment_filepath=None, experiment_name=None, 
                                            num_tasks=8, target_ind=1, use_coarse_data=True, 
    ...
    num_classes_fine_tasks = [102, 67, 200, 196, 70, 10, 62, 10]
    total_classes_fine = sum(num_classes_fine_tasks)
    seq_tasks_fine = [[i for i in range(j)] for j in num_classes_fine_tasks]
    ...
    dset_prep={'use_coarse_data':use_coarse_data, 'scenario_classif':target_ind, 'homog_ind':2, 'total_classes_coarse':8, 'total_classes_fine': total_classes_fine,\
        'num_classes_fine_tasks':num_classes_fine_tasks}
    ...
    return train_datasets, train_holdout_datasets, val_datasets, test_datasets, seq_tasks, seq_tasks_fine, dset_prep
