...
class CurrentTask():
    ...
    def __getitem__(self, idx):
        ...
        im, y_fine, y_coarse, _ = self.dsets[dset_id].__getitem__(idx_dset)
        ...
        if self.returnIDX:
            return im, class_lbl, y_coarse, novelty_lbl, idx
            
        return im, class_lbl, y_coarse, novelty_lbl
...
class PseudolabelDset():
    ...
    def __getitem__(self, idx):                
        ...
        im, _, _,_ = self.dset.__getitem__(idx) 
...
class NovelTask():
    def __init__(self, task, num_current_classes, current_dset, \
        ...
        """ 
        Use Ground Truth labels to train forward .... instead of predicted 
    ...
    def __getitem__(self, idx):
        ...
        if self.task==0:
            im, y_fine, task_ID, _ = self.dset.__getitem__(idx)
            novelty_gt = 1
            if self.use_coarse:
                class_lbl = task_ID
            else:
            ...
            im, class_lbl, task_ID, novelty_gt = self.dset.__getitem__(idx)
            # class_lbl = self.num_current_classes


        if self.returnGT:
            return im, class_lbl, task_ID, class_lbl, novelty_gt
            
        return im, class_lbl, task_ID



class ValidationDset():
    def __init__(self, val_dsets, use_coarse=False, returnIDX=False):
        """ 
    ...
    def __getitem__(self, idx):
        ...
        if self.returnIDX:
            return im, class_lbl, dset_id, dset_id, idx
            
        return im, class_lbl, dset_id, dset_id
