...
def train_main_epoch(epoch, loader_new, loader_old, model_main, optimizer, scheduler, OOD_class, weight_old=0.5,\
                                feature_name='base.8', latent_name=None, latent_criterion=None, weight_latent=0, quantizer=None,  \
                                target_ind=1, cuda=True, display_freq=10, finetune_backbone=False, cut_epoch_short=False, device=0):

    model_main.model.train()
    if finetune_backbone==False:
        model_main.model.base.train(False)
        model_main.base_freeze = True
    else:
        model_main.model.base.train(True)
        model_main.base_freeze = False

    print('Number batches: ', len(loader_new))
    
        
    for batch_idx in range(len(loader_new)):
        ...
        # make sure this doision does not interfere with backprop in finetuning case 
